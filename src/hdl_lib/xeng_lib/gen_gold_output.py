...
d = d.reshape([n_ants, acc_len, n_pols, p_samples, 2])
d_uint = d_uint.reshape([n_ants, acc_len, n_pols, p_samples, 2])

d_c = n.zeros([n_ants, acc_len, n_pols, p_samples], dtype=complex)
d_uint_c = n.zeros([n_ants, acc_len, n_pols, p_samples], dtype=complex)
...
for a in range(n_ants):
    for b in range(n_ants):
        for p in range(n_pols):
            corr = n.sum(d_c[a,:,p,:] * n.conj(d_c[b,:,p,:])) # XX only
            corr_uint = n.sum(d_uint_c[a,:,p,:] * n.conj(d_uint_c[b,:,p,:]))
            #if (n.imag(corr) < 0):
            #    f.write("(%2d,%2d) %8d%8di,  %8d%8di,  %8d%8di\n" %(a,b,int(n.real(corr_uint))&output_mask,int(n.imag(corr_uint))&output_mask,n.real(corr), n.imag(corr), int(n.real(corr))&output_mask, int(n.imag(corr))&output_mask))
            #else:
            f.write("(%2d,%2d,%d) %8d + %8di,  %8d + %8di,  %8d + %8di\n" %(a,b,p,int(n.real(corr_uint))&output_mask,int(n.imag(corr_uint))&output_mask,n.real(corr), n.imag(corr),int(n.real(corr))&output_mask, int(n.imag(corr))&output_mask))
